...
const transformBookingForDetails = (booking: Booking, currentUser: any) => {
  ...
  return {
    ...
    ownerId: booking.owner?.id || currentUser?.id || '',
    ownerName: booking.owner 
      ? `${booking.owner.firstName || ''} ${booking.owner.lastName || ''}`.trim() || currentUser?.user_metadata?.full_name || 'Propriétaire'
      : (currentUser?.user_metadata?.full_name || currentUser?.email?.split('@')[0] || 'Propriétaire'),
    ownerEmail: booking.owner?.email || currentUser?.email || '',
    ownerPhone: booking.owner?.phone || currentUser?.user_metadata?.phone || '',
    ownerAvatarUrl: booking.owner?.avatarUrl || currentUser?.user_metadata?.avatar_url || '',
    
    // Champs requis par BookingDetails
    dailyRate: durationDays > 0 ? (booking.totalAmount || 0) / durationDays : 0,
    durationDays: durationDays,
    serviceFee: (booking.totalAmount || 0) * 0.1, // 10% de frais de service
    depositAmount: 0,
    ...
    check_in_photos: [],
    check_out_photos: [],
    checkInOutStatus: 'not_started' as const,
    contactShared: false
  };
};
  ...
  const fetchBookings = async () => {
    try {
      const { data, error } = await supabase
        ...
        `)
        .or(`host_id.eq.${user?.id},owner_id.eq.${user?.id}`)
      ...
      const transformedBookings: Booking[] = data.map(booking => {
        ...
        // Utiliser vehicle ou car selon ce qui est disponible
        const vehicle = booking.vehicle || booking.car;
        
        // Utiliser host ou owner selon ce qui est disponible
        const owner = booking.host || booking.owner;
          ...
          {filteredBookings.length === 0 ? (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-10 text-center">
                ...
                <h3 className="font-semibold text-lg">Aucune réservation trouvée</h3>
